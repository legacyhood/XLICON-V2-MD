...
async function getDb() {
    ...
    try {
        ...
        console.error('[warn] MongoDB connect failed:', e.message);
        return null;
    }
}
...
async function getWarns(groupId, userId) {
    ...
    const doc = await database.collection('warnings').findOne({ groupId, userId });
    return doc || { count: 0, reasons: [] };
}
...
async function addWarn(groupId, userId, reason) {
    ...
    const result = await database.collection('warnings').findOneAndUpdate(
        ...
        {
            $inc: { count: 1 },
            $push: { reasons: { reason: reason || 'No reason given', at: new Date() } },
...
module.exports = {
    ...
    async execute(sock, m, args) {
        ...
        if (!botMember?.admin) return m.reply('❌ I need to be an admin to warn members.');
        ...
        if (count >= WARN_LIMIT) {
            ...
            await sock.sendMessage(m.from, {
                text:
`⛔ @${target.split('@')[0]} has been *auto-kicked*!\n\n` +
...
`${remaining > 0 ? `⚠️ ${remaining} more warn${remaining === 1 ? '' : 's'} before auto-kick.` : ''}`,
                mentions: [target]
            });
        }
        await m.react('✅');
