// ── In-memory spam tracker & settings cache ─────────────────────────────────
const spamTracker = new Map();
const settingsCache = new Map(); // groupId → { antispam: bool, cachedAt: Date }
...
// Cache group metadata (avoid calling groupMetadata on every message)
...
async function getGroupMeta(sock, groupId) {
    ...
    try {
        ...
        metaCache.set(groupId, { meta, cachedAt: Date.now() });
        return meta;
    } catch(_) { return null; }
}
...
module.exports = {
    ...
    async execute(sock, m, args) {
        ...
        const meta = await getGroupMeta(sock, m.from);
        if (!meta?.participants.find(p => p.id.replace(/:\d+@/,'@') === botId)?.admin)
            return m.reply('❌ I need to be an admin.');
    ...
    async onMessage(sock, m) {
        ...
        entry.ts.push(now);
        entry.keys.push(m.key);

        if (entry.ts.length >= SPAM_MSG_LIMIT) {
            const toDelete = [...entry.keys];
            entry.ts = []; entry.keys = [];
            ...
            if (count >= WARN_LIMIT) {
                ...
                await sock.sendMessage(m.from, { text:`⛔ @${sc.split('@')[0]} was *kicked* for spamming! (${WARN_LIMIT}/${WARN_LIMIT} warns)`, mentions:[sc] });
                await clearWarns(m.from, sc);
            } else {
                await sock.sendMessage(m.from, { text:`🚨 *SPAM DETECTED*\n👤 @${sc.split('@')[0]}\n🗑️ ${toDelete.length} messages deleted\n⚠️ Warns: *${count}/${WARN_LIMIT}*`, mentions:[sc] });
            }
        }
        // Prune old tracker entries
        if (spamTracker.size > 500) {
            for (const [k,v] of spamTracker) if (!v.ts.some(t=>now-t<win*2)) spamTracker.delete(k);
        }
    }
};
