...
async function getSetting(groupId) {
    const db = await getDb(); if(!db) return {};
    return await db.collection('group_settings').findOne({ groupId }) || {};
}
async function setSetting(groupId, update) {
    const db = await getDb(); if(!db) return;
    await db.collection('group_settings').updateOne({ groupId }, { $set: { ...update, updatedAt: new Date() } }, { upsert: true });
}
...
module.exports = {
    ...
    async execute(sock, m, args) {
        ...
        const isAdmin = meta?.participants.find(p => p.id.replace(/:\d+@/, '@') === botId)?.admin;
        if (!isAdmin) return m.reply('❌ I need to be an admin.');

        const sub = args[0]?.toLowerCase();
        const text = args.slice(1).join(' ').trim();

        if (sub === 'on') {
            await setSetting(m.from, { welcomeOn: true });
            return m.reply('✅ Welcome messages *enabled*!\n\nCustomise: _.welcome msg Your welcome text_\nVariables: {name} {group} {count}');
        }
        if (sub === 'off') {
            await setSetting(m.from, { welcomeOn: false, goodbyeOn: false });
            return m.reply('✅ Welcome/goodbye messages *disabled*.');
        }
        if (sub === 'msg' && text) {
            await setSetting(m.from, { welcomeMsg: text });
            return m.reply(`✅ Welcome message set:\n\n_${text}_`);
        }
        if (sub === 'goodbye' && text) {
            await setSetting(m.from, { goodbyeMsg: text, goodbyeOn: true });
            return m.reply(`✅ Goodbye message set:\n\n_${text}_`);
        }
        if (sub === 'goodbye' && !text) {
            await setSetting(m.from, { goodbyeOn: true });
            return m.reply('✅ Goodbye messages *enabled*!');
        }
        if (sub === 'test') {
            ...
            const rendered = welcomeMsg
                ...
                .replace('{group}', meta?.subject || 'this group')
                .replace('{count}', meta?.participants?.length || '?')
                .replace('{numjid}', m.sender?.split('@')[0]);
...
*Variables:* {name} {group} {count} {numjid}`
    ...
    // Called from index.js on group-participants.update
    async onGroupUpdate(sock, update) {
        ...
        for (const jid of participants) {
            ...
            if (action === 'add' && settings.welcomeOn !== false) {
                const msg = (settings.welcomeMsg || '👋 Welcome to *{group}*, @{numjid}!\n\nWe are now *{count}* members strong. Enjoy your stay! 🎉')
                    .replace('{name}', num).replace('{group}', group).replace('{count}', count).replace('{numjid}', num);
                await sock.sendMessage(id, { text: msg, mentions: [jid] }).catch(() => {});
            }
            if (action === 'remove' && settings.goodbyeOn) {
                const msg = (settings.goodbyeMsg || '👋 Goodbye @{numjid}! We will miss you. Now *{count}* members remaining.')
                    .replace('{name}', num).replace('{group}', group).replace('{count}', count).replace('{numjid}', num);
                await sock.sendMessage(id, { text: msg, mentions: [jid] }).catch(() => {});
            }
        }
