...
const server = http.createServer((req, res) => {
    if (req.url === '/' || req.url === '/qr') {
        ...
        res.end(`<!DOCTYPE html><html><head><title>XLICON-V2-MD</title><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font-family:sans-serif;background:#111;color:#eee;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px;box-sizing:border-box}h1{color:#25d366}#qr img{max-width:280px;border:6px solid #25d366;border-radius:12px;background:#fff;padding:10px}#qr .msg{font-size:1.1em;color:#25d366;padding:30px;border:2px dashed #25d366;border-radius:12px;text-align:center}#status{margin-top:16px;padding:8px 20px;border-radius:20px;font-weight:bold}.connected{background:#25d366;color:#000}.waiting{background:#333;color:#aaa}#sb button{margin-top:8px;padding:10px 20px;background:#25d366;color:#000;border:none;border-radius:8px;cursor:pointer;font-weight:bold}#resetBtn{margin-top:16px;padding:10px 24px;background:#e53935;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:bold;font-size:14px;display:block;width:100%;max-width:260px}</style></head><body><h1>🤖 XLICON-V2-MD</h1><div id="qr"><div class="msg">⏳ Loading...</div></div><div id="status" class="waiting">Starting...</div><button id="resetBtn" onclick="resetSession()">🔄 Reset Session (New QR)</button><a href="/api/session" target="_blank" style="margin-top:10px;padding:10px 24px;background:#1565c0;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:bold;font-size:14px;display:block;width:100%;max-width:260px;text-align:center;text-decoration:none;box-sizing:border-box">📋 Get Session ID</a><p id="resetMsg" style="color:#e53935;font-size:13px;display:none">Clearing session... reload in 5 seconds</p><script>async function resetSession(){if(!confirm('Clear the current session and show a fresh QR code?'))return;document.getElementById('resetMsg').style.display='block';await fetch('/api/reset',{method:'POST'});setTimeout(()=>location.reload(),5000);}async function poll(){try{const r=await fetch('/api/status');const d=await r.json();const q=document.getElementById('qr');const s=document.getElementById('status');if(d.status==='connected'){q.innerHTML='<div class="msg">✅ Bot Connected!</div>';s.textContent='Connected ✅';s.className='connected';}else if(d.hasQR&&d.qr){q.innerHTML='<img src="'+d.qr+'" alt="QR">';s.textContent='Scan QR code ↑';s.className='waiting';}else{q.innerHTML='<div class="msg">⏳ Connecting...</div>';s.textContent=d.status;s.className='waiting';}}catch(e){}setTimeout(poll,4000);}poll();</script></body></html>`);
